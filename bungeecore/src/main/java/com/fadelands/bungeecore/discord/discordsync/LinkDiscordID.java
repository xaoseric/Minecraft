                    ...
                    try(PreparedStatement st = connection.prepareStatement("SELECT * FROM fadelands_discord_regs WHERE player_uuid='" + player.getUniqueId() + "'")) {
                        try (ResultSet rs = st.executeQuery()) {
                            ...
                            if (rs.next()) {
                                ...
                                String let = com.fadelands.bungeecore.discord.discordsync.DiscordSecretKeyGenerator.digits + "ACEFGHJKLMNPQRUVWXYabcdefhijkprstuvwx";
                                com.fadelands.bungeecore.discord.discordsync.DiscordSecretKeyGenerator output = new com.fadelands.bungeecore.discord.discordsync.DiscordSecretKeyGenerator(9, new SecureRandom(), let);
